...
export default function CertificateApprovalsPage() {
  return (
    <AccessControlWrapper pagePath="approvals/certificate">
      <CertificateApprovalsContent />
    </AccessControlWrapper>
  );
}

function CertificateApprovalsContent() {
  const { user } = useAuth();
  ...
  const fetchApprovals = async () => {
    try {
      ...
      console.error('Failed to fetch certificate approvals:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-4 md:p-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Review and approve pending certificate requests</p>
          </div>
        </div>
