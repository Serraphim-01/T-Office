/**
 * Cleanup Multi Users Script
 * Completely deletes all users created by the multi user creation script
 */
...
async function deleteUsersFromTestData(dbClient) {
  console.log('🗑️ Starting deletion of users from test data file...');
...
async function deleteUsersByPattern(dbClient) {
  console.log('📋 Using email pattern to find test users...');
...
async function main() {
  console.log('=== Multi User Cleanup Script ===\n');
  ...
  try {
    ...
    // First try to delete from test data file
    const deletedFromTestData = await deleteUsersFromTestData(dbClient);
    totalDeleted += deletedFromTestData;
    
    // Then try to delete by pattern (catch any remaining test users)
    const deletedByPattern = await deleteUsersByPattern(dbClient);
    totalDeleted += deletedByPattern;
    
    console.log('\n🎉 Multi user cleanup completed successfully!');
    console.log(`📊 Total users deleted: ${totalDeleted}`);
    
  } catch (error) {
    console.error('\n✗ Multi user cleanup failed:', error.message);
    process.exit(1);
  } finally {
    if (dbClient) {
      await dbClient.end();
...
export { main, deleteUsersFromTestData, deleteUsersByPattern };